<template>
  ...
  <div class="app-container">
    <div class="head">
      <el-select v-model="value" placeholder="全部文件">
        <el-option
      ...
      <el-button class="filter-item" type="primary" style="margin-left: 1rem" @click="piliangtongguo">
        批量通过</el-button
      >
      <el-button class="filter-item" type="danger" style="margin-left: 1rem" @click="piliangbohuiDialog = true">
        批量驳回</el-button
      >
    </div>
    ...
    >
      ...
      <el-table-column label="状态" align="center">
        <template slot-scope="{ row }">
          <el-tag type="success" v-if="row.state == 1">已通过</el-tag>
          <el-tag type="info" v-else-if="row.state == 2">待审核</el-tag>
          <el-tag type="danger" v-else-if="row.state == 3">已驳回</el-tag>
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="驳回理由">
          <el-input type="textarea" v-model="addDepartmentName"  placeholder="请输入驳回理由,选填"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog
      :close-on-click-modal="false"
      :visible="piliangbohuiDialog"
      width="40rem"
      title="批量驳回"
      :before-close="closePiliangBohuiDialog"
    >
      <el-form
        label-position="left"
        label-width="140px"
      ...
      <div slot="footer" class="dialog-footer">
        <!-- 取消按钮 -->
        <el-button @click="piliangbohuiDialog = false" style="margin-right: 0.2rem"
          >取消</el-button
        >

        <!-- 确认按钮 -->
        <el-button type="primary" @click="piliangbohuiDialog = false">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script>
import {} from "../../api/auditManagement/auditManagement";
...
export default {
  data() {
    return {
      ...
      bohuiDialog: false,
      addDepartmentName:"",
      piliangbohuiDialog:false,
    };
  },
  methods: {
    onTongguo(id) {
      this.$confirm("是否通过此条审核?", "提示", {
    ...
    closePiliangBohuiDialog() {
      this.piliangbohuiDialog = false;
    },

    piliangtongguo(){
      this.$confirm("是否批量通过6条审核?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
