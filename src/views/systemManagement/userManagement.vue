  ...
  <div class="app-container">
    ...
    <el-table :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column label="时间" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.createDate.year }}-{{ row.createDate.monthValue }}-{{ row.createDate.dayOfMonth }}  {{ row.createDate.hour }}-{{ row.createDate.minute }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    //   请求所有数据
    async queryAllUser() {
      var res = await reqQueryAllUser(1, 10, "", "", "", "", "");
      console.log(res);
      if (res.code == 0) {
    ...
    async createUSer() {
      ...
      var res = await reqCreateUser(this.name,this.rolelds,this.idNo,this.no,this.passCode,this.phone,this.userName)
      if(res.code == 0){
        this.createUserDialog = false
        this.$message("添加成功")
        this.name = ""
